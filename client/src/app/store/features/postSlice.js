...
export const deletePost = createAsyncThunk(
    ...
    async (props, thunkAPI) => {
        ...
        const { dispatch, fulfillWithValue, getState, rejectWithValue } =
            thunkAPI;
        const {
            user: { isGuest }
        } = getState();
...
export const editComment = createAsyncThunk(
    ...
    async (props, thunkAPI) => {
        ...
        const { dispatch, rejectWithValue, getState, fulfillWithValue } =
            thunkAPI;
        const {
            user: { isGuest }
        } = getState();
        ...
        builder
            ...
            .addCase(likePost.fulfilled, (state, { payload }) => {
                ...
                if (add) {
                    ...
                    _post &&
                        (_post.likes = _post.likes.filter(
                            (ele) => ele !== userId
                        ));
                }
            })
            .addCase(update.type, (state, action) => {
                ...
                state.allPosts.posts = state.allPosts.posts.map((post) => {
                    if (post.createdBy === id) {
                        ...
                        const updatedImage =
                            profileImage || post.userDetails.image;
                        return {
                            ...post,
                            userDetails: {
                                name: updatedName,
